...
/*
    ...
    Provider로 부터 현재 값을 읽어올린다. 이 때 적절한 provider를 찾지 못할 때 쓰이는 값이 defaultValue 이다.
*/

const DarkModeContext = createContext(null); // 여기서 defaultValue 는 null 이다.
...
const Footer = () => {
    ...
    const toggleHeandler = () => setIsDark(!isDark);
    ...
    return (
        ...
        >
            <button onClick={toggleHeandler}>{isDark ? 'black' : 'white'}</button>
            useContext aPP
        </footer>
    );
}
